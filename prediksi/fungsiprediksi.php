...
//Tambah Data Training
    ...
    if(isset($_POST['daftar'])){
        ...
        // SELECT * FROM `rule` WHERE `Aturan` LIKE "%Kedisiplinan='Cukup Baik'%" AND `Aturan` LIKE "%Kerja_Sama='Cukup'%" AND `Aturan` LIKE "%IQ='71 - 80'%" AND `Aturan` LIKE "%Keaktifan='Cukup'%" AND `Aturan` LIKE "%Rata_rata_Nilai_US='76-80'%";
        ...
        if( $query ) {
			global $idpengguna;
			global $koneksi;
			// $rulecocok = "IQ=($iq) AND Nilai_PAI=($pai) AND Rata_rata_Nilai_US=($nilaius) AND
			// 	Keaktifan=($keaktifan) AND Kedisiplinan=($kedisiplinan) AND Kerja_Sama=($kerjasama) AND
			// 	Ketunaan=($ketunaan)";
			...
			while($row=mysqli_fetch_array($sql)){
				...
				// $rule=str_replace("Nilai_PAI","'$pai'",$rule);
			...
			if ($keputusan == '') {
				...
				echo "<h4><center>Rule yang terpilih adalah rule ke-".$rule."</center></h4>";			
...
<body>
	...
	<div class="modal-dialog">
		...
		<div class="modal-footer">
			<button type="button" onclick="location.href='hasil_prediksi.php'" class="btn btn-secondary">Tutup</button>
      	</div>    
		</div>
	</div>
	</div>
